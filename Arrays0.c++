...
//  b = a;                                     // error: invalid array assignment
    ...
    assert(a[1] == v);
    f(a); // removes the consty-ness of a, ie a++ ++*a would increment the second index
    assert(a[1] == v + 2);
...
//  B* const a = new A[10];                      // error: invalid conversion from ‘A*’ to ‘B*’
    A* const a = new B[10];                      // dangerous, doesn't realize its point to B's and B's may be bigger than A
    assert(a[0].f() == "A::f");
